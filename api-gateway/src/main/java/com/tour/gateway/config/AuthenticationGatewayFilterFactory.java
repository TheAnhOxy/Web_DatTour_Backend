...
@Component
@Slf4j
// 1. Tên Class phải có hậu tố GatewayFilterFactory
public class AuthenticationGatewayFilterFactory extends AbstractGatewayFilterFactory<AuthenticationGatewayFilterFactory.Config> {

    private final WebClient.Builder webClientBuilder;
    private final ObjectMapper objectMapper;

    // 2. Constructor duy nhất, tên PHẢI trùng khớp với Class
    ...
    // 3. Class Config (chỉ cần 1 cái nội bộ)
    public static class Config {}

    @Override
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            log.info(">>>> Gateway Filter đang xử lý request: {}", exchange.getRequest().getPath());
            ...
            return webClientBuilder.build()
                    ...
                    .flatMap(apiResponse -> {
                        ...
                        if (introspect != null && introspect.isValid()) {
                            log.info("Token hợp lệ, cho phép qua cửa!");
    ...
    private Mono<Void> onError(ServerWebExchange exchange, String err, HttpStatus httpStatus) {
        log.error("Lỗi Gateway Auth: {}", err);
